...
Donc si nous commençons avec Fonction(1), comme 1 n’est pas égal à 3 alors nous allons ajouter la valeur de l’indice 1(10) à la valeur retournée par Fonction(2) soit Fonction(n+1) et comme n est egale à 1 à ce stade,je vous laisse deviner la prochaine fonction appelée(Bon je vous spoile c'est bien Fonction(2)). Mais nous n’avons toujours pas la valeur de Fonction(2) qui doit à son tour être appelée pour retourner une valeur.

La Fonction(1) est maintenant en stand-by en attendant la valeur de Fonction(2).
...
C’est fini ? Restez encore, nous avons toujours des fonctions en stand-by. C’est-à-dire les fonctions qui ne sont toujours pas terminées et qui attendent le retour d’une fonction soit Fonction(1) et Fonction(2).
